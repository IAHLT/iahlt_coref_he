...
from fastcoref.utilities.collate import PadCollator, LeftOversCollator

from iahlt_coref_he.util import extract_clusters, extract_mentions_to_clusters, mask_tensor
from iahlt_coref_he.util import get_pronoun_id, get_category_id
from iahlt_coref_he.consts import STOPWORDS
from iahlt_coref_he.args import parse_args
from iahlt_coref_he.tokenizer import Tokenizer
...
class S2E(BertPreTrainedModel):
    def __init__(self, config, args=parse_args()):
    ...
    def forward(self, batch, gold_clusters=None, return_all_outputs=False):
        ...
        if return_all_outputs:
            # Added final_logits twice instead of mention_logits (fastcoref expects it), original version is (mention_start_ids, mention_end_ids, final_logits) 
            outputs = (mention_start_ids, mention_end_ids, final_logits, final_logits)
        else:
            outputs = tuple()
...
class S2ECoref(CorefModel):
    def __init__(self, model_name_or_path=os.path.join('externals', 'coref_model'), device=None, nlp=None, enable_progress_bar=True):
        super().__init__(model_name_or_path, S2E, PadCollator, enable_progress_bar, device, nlp)


class CorefPipeline:
    def __init__(self, tokenizer_model_path, coref_model_path, device='cuda') -> None:
        self.device = torch.device(device if torch.cuda.is_available() else "cpu")
        self.tokenizer = Tokenizer(tokenizer_model_path)
        self.s2e = S2ECoref(model_name_or_path=coref_model_path, device=self.device, nlp=None, enable_progress_bar=False)
    
    def predict(self, texts):
        returned_data = {}
        tokens = [self.tokenizer.tokenize(text=text) for text in texts]
        corefs = self.s2e.predict(texts=tokens, is_split_into_words=True)
        returned_data['tokens'] = tokens
        returned_data['mentions'] = [r.get_clusters(as_strings=False) for r in corefs]
        returned_data['mentions_str'] = [r.get_clusters() for r in corefs]
